...
from rag_fns.setup_load import import_data
...
def make_app(n_results: int) -> None:
    ...
    with st.sidebar:
        ...
        if use_oai:
            ...
            st.markdown(
                """If you don't have an OpenAI API key, you can sign up [here](https://platform.openai.com/account/api-keys)."""
            )
        else:
            openai_api_key = None
            st.markdown(
                "Rate limits may be applied to this app due to its use of [Groq](https://groq.com/)"
            )
    ...
    if use_oai and not openai_api_key:
        with chat_container:
            st.warning(
                "Please either put in an OpenAI API Key (and then press enter) or just use Llama3-8b, which isn't quite as good but is good enough."
            )
    else:
